...
fn catalog_generation_is_deterministic_and_opaque() {
    ...
    let generated = fs::read_to_string(root.path().join("tools/harness-runtime/src/generated_postgres.rs")).unwrap();
    ...
    let first = generated;
    xtask::image_catalog::generate(root.path()).unwrap();
    assert_eq!(first, fs::read_to_string(root.path().join("tools/harness-runtime/src/generated_postgres.rs")).unwrap());
    fs::write(root.path().join("tools/harness-runtime/src/generated_postgres.rs"), "drift").unwrap();
