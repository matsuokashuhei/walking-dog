...
- Expo SDK 56 では `jest-expo@56.0.4` / `@react-native/jest-preset@0.85.3` が Jest 29 系に依存し、`eslint-config-expo@56.0.4` 配下の `eslint-plugin-react` / `eslint-plugin-import` は ESLint 10 をまだサポートしない。Dependabot の dev tooling 更新では `jest` / `@types/jest` / `eslint` / `react-test-renderer` を個別に上げず、`react-test-renderer` は `react` と同じ 19.2.3 に揃える。i18next 26 以降で同期初期化が必要な Jest setup は `initImmediate` ではなく `initAsync: false` を使う。
...
- 複数 worktree で Expo/Metro を起動していると 8081 の既存プロセスが別 worktree を指していることがある。現在の worktree を視覚確認するときは `ps -p <pid>` で対象プロセスの cwd/command を確認し、別 worktree の Metro に接続して確認済み扱いにしない。
...
- WatchConnectivity の受信は Watch UI の `@StateObject` だけに持たせない。`updateApplicationContext` / `transferUserInfo` は Watch app の background wake で `WKWatchConnectivityRefreshBackgroundTask` を発生させるため、app lifecycle 側で `WCSessionDelegate` と background task completion を所有し、`activationDidComplete` 後に `receivedApplicationContext` も反映する。
...
- Me タブの route file は `app/(tabs)/user/index.tsx` とし、ユーザーのプロフィールと散歩貢献を主表示する画面として扱う。表示設定・法務リンク・サインアウトなどの設定一覧は Me 下部のリンクから開く `/(tabs)/user/settings` に置き、ユーザー編集は `/(tabs)/user/edit` に置く。Me タブ上部にプロフィールカードから別プロフィール詳細へ遷移する旧構造を戻さない。
