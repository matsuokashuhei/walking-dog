...
- メールアドレス変更・パスワード変更などのアカウント認証 mutation は、画面から Cognito を直接呼ばず `lib/auth/api.ts` と `authenticatedRequest` 経由で Rust API の GraphQL mutation に集約する。メール変更は `changeEmail` で確認コードを送信し、`confirmEmailChange` で完了する 2 段階フローにする。
