...
- React Native の `PanResponder` を Jest / React Native Testing Library で検証するときは、`fireEvent(..., 'responderMove', {}, { dx })` のように任意の gestureState を渡さない。RN 側が `touchHistory` から `gestureState.dx` を計算するため、`onResponderGrant` / `onResponderMove` / `onResponderRelease` に `touchBank`・`mostRecentTimeStamp`・`numberActiveTouches` を含む event を渡して検証する。
