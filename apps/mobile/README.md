...
|---|---|---|---|
| `npm run ios:sim:local` | Simulator | `http://localhost:<dev-stack API port>` | Debug |
...
| `npm run ios:dev:local` | Device | `http://localhost:<dev-stack API port>`（実機はLAN IPへ差し替え） | Debug |
...
### Local (`.harness-runs/dev-stack/env.json` のAPIポート) — フロント開発
