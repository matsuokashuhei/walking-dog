import { render, screen } from '@testing-library/react-native';
import { Text } from 'react-native';
import { typography } from '@/theme/tokens';
import { SectionHeader } from './SectionHeader';

jest.mock('@/hooks/use-color-scheme', () => ({
  useColorScheme: () => 'light',
}));

describe('SectionHeader', () => {
  it('renders label text', () => {
    render(<SectionHeader label="Your pack" />);
    expect(screen.getByText('Your pack')).toBeTruthy();
  });

  it('applies the tokenized metric label typography', () => {
    render(<SectionHeader label="Legal" testID="header" />);
    const node = screen.getByTestId('header');
    const flat = flatten(node.props.style);
    expect(flat.textTransform).toBe('uppercase');
    expect(flat.fontSize).toBe(typography.metricLabel.fontSize);
    expect(flat.letterSpacing).toBe(typography.metricLabel.letterSpacing);
  });

  it('renders trailing slot next to the label', () => {
    render(
      <SectionHeader
        label="Who's coming?"
        trailing={<Text>Select all</Text>}
      />,
    );
    expect(screen.getByText("Who's coming?")).toBeTruthy();
    expect(screen.getByText('Select all')).toBeTruthy();
  });
});

type Flat = Record<string, unknown>;
function flatten(style: unknown): Flat {
  const arr = (Array.isArray(style) ? style : [style]).flat() as (Flat | undefined)[];
  return arr.reduce<Flat>((acc, s) => ({ ...acc, ...(s ?? {}) }), {});
}
